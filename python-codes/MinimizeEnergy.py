...
from utilities import compute_force_vector, compute_potential #, wrap_in_box
    ...
    def run(self):
        ...
        for self.step in range(0, self.maximum_steps+1):
            ...
            if hasattr(self, 'MaxF') is False: # If self.MaxF does not exist yet, calculate it
                forces = compute_force_vector(self.neighbor_lists,
                                    self.atoms_positions, self.box_size,
                                    self.sigma_ij_list, self.epsilon_ij_list)
            ...
            if trial_Epot < init_Epot: # accept new position
                ...
                forces = compute_force_vector(self.neighbor_lists,
                                              self.atoms_positions, self.box_size,
                                              self.sigma_ij_list, self.epsilon_ij_list) # calculate the new max force and save it
                self.MaxF = np.max(np.abs(forces))
