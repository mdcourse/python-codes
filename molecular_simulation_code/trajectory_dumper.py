...
def update_dump_file(code, filename, velocity=False):
    if code.dumping_period is not None:
        if code.step % code.dumping_period == 0:
            ...
            box_bounds = code.box_bounds*code.ref_length # Angstrom
            atoms_positions = code.positions*code.ref_length # Angstrom
            atom_types = code.atom_types
            if velocity:
            ...
            for dim in np.arange(3):
                f.write(str(box_bounds[dim][0].magnitude) + " "
                        + str(box_bounds[dim][1].magnitude) + "\n")
            cpt = 1
            if velocity:
                f.write("ITEM: ATOMS id type x y z vx vy vz\n")
                characters = "%d %d %.3f %.3f %.3f %.3f %.3f %.3f %s"
                for type, xyz, vxyz in zip(atom_types,
                ...
                for type, xyz in zip(atom_types,
                                    atoms_positions.magnitude):
                    v = [cpt, type, xyz[0], xyz[1], xyz[2]]
